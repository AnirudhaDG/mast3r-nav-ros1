...
episode_names_all = natsorted(Path("./data/hm3d_iin_val/").iterdir())#[:54:3]
# episode_identifiers = ['_'.join(ed.name.split('_')[:5]) for ed in episode_names_all]

# base_dir = Path(sys.argv[1]) # out/results/strech_maps/

# test_one_third = False
# if len(sys.argv) > 2:
#     test_one_third = int(sys.argv[2]) == 1
...
results_dirs = [
    subdir for subdir in args.base_dir.rglob('*')  # Recursively iterate through all entries
    if subdir.is_dir() and pattern.match(subdir.name)  # Check if it's a directory and matches the pattern
]
...
method_pairs = [
    # ["ITL_modelBB3_sh131/", "ITL_modelBB3Sh4_sh4/"],
    # ["TL_learnt_mapS21LGInfD_cullMapFS/", "TL_learnt_mapS21LGInfD_cullMapFS_sh131/"],
    # ["GTTM_learnt/", "GTTM_learnt_sh131/"],
    # ["GNM/", "GNM_sh131/"],
    # ["TL_pixnav_mapS21LGInfD_cullMapFS/", "TL_pixnav_mapS21LGInfD_cullMapFS_sh131/"],
    # ["TL_robohop_mapS21LGInfD_cullMapFS/", "TL_robohop_mapS21LGInfD_cullMapFS_sh131/"],
    # ["GTTM_robohop/", "GTTM_robohop_sh131/"],
    ["", ""]
]

for method_pair in method_pairs:
    method, method_2 = method_pair
    print(f"Processing method_pair", method, method_2)
    paper_results = {}

    loopCount = 0
    for results_dir in tqdm(results_dirs):
        episode_dirs = natsorted([d for d in results_dir.iterdir() if d.is_dir() and 'summary' not in d.name])
        if args.test_one_third:
            episode_dirs = episode_dirs[::3]
        if len(episode_dirs) == 0:
            continue
        if method not in str(results_dir) and method_2 not in str(results_dir):
            continue
        ...
        # episode_blacklist += ['4ok3usBNeis_0000000_chair_8_', 'svBbv1Pavdk_0000000_plant_7_', 'DYehNKdT76V_0000000_toilet_115_', 'q5QZSEeHe5g_0000000_plant_32_']
        print(f"{task_type=}")
        ...
        for ei, ed in enumerate(episode_dirs):
            ...
            metadata_dict = {m.split('=')[0]: m.split('=')[1] for m in metadata_vals if '=' in m}
            ...
            success_status =  metadata_dict['success_status'] #[m for m in metadata if 'success_status' in m]
            if len(success_status) == "":
                ...
                if report_collisions or compute_spl:
                    ...
                    if compute_spl:
                        ...
                        path_length = np.linalg.norm(xz[1:] - xz[:-1], axis=1).sum() + remain_distance

                if report_collisions: avg_collisions_list.append(collisions_num)
                if compute_spl:
        ...
        if report_collisions:
            print(f"MeanAvg Collisions of Success Runs: {np.mean(avg_collisions_list):.2f}")
        if compute_spl:
    ...
    # # assert loopCount == 8, f"{loopCount=}"
    # print(f"Processed methods:", paper_results['original'].keys())
    # task_types = ['original', 'alt_goal_v2', 'via_alt_goal', 'original_reverse']
    # metrics = ['spl', 'soft_spl']
    # # [print(metric[:5], end='\t') for metric in metrics]
    # # print()
    # for task_type in task_types:
    #     # average over methods
    #     for metric in metrics:
    #         avg_sensor_heights = np.mean([paper_results[task_type][method][metric] for method in paper_results[task_type].keys()])
    #         print(f"{avg_sensor_heights:.2f} &", end='\t')
    # print()
